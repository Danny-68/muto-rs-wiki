...
class Yahboom_OLED:
    ...
    # 读取电池电量百分比
    # Read battery percentage — shared file first, serial fallback
    def getBattery(self):
        try:
            with open('/tmp/battery_pct', 'r') as f:
                pct = int(f.read().strip())
            try:
                with open('/tmp/battery_volt', 'r') as f:
                    volt = f.read().strip()
                return "B:%d%% %sV" % (pct, volt)
            except Exception:
                return "BATT:%d%%" % pct
        except Exception:
            pass
        try:
            import sys
            sys.path.insert(0, '/home/pi/yahboomcar_ros2_ws/software/MutoLib')
            from MutoLib.MutoLib import Muto
            bot = Muto()
            pct = bot.read_battery(voltage=False)
            volt = bot.read_battery()
            bot.ser.close()
            return "B:%d%% %.1fV" % (int(pct), volt)
        except Exception:
            return "BATT:---"
