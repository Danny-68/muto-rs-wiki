...
**Update (zelfde dag, tweede analyse):** een onafhankelijke analyse van de gebruiker stelt een systematische schaalfout in rf2o zelf voor (i.p.v. een architectuurprobleem), met als voorgestelde vervolgtest: rf2o's rauwe `/odom` (zonder EKF) direct vergelijken met een fysiek gemeten hoek bij 90°/180°/360° in beide richtingen, plus de LD06 **losgekoppeld van de lopende MUTO** (handmatig/draaiplateau) testen om gait-vibratie volledig uit te sluiten. **Belangrijk verschil met onze eigen metingen:** de aangeleverde voorbeeldcijfers gaan uit van een ONDERSCHATTING (rf2o rapporteert ~2/3 van de werkelijke hoek, bv. 360°→240°), terwijl onze eigen `isolate_odom_vs_imu.py`-test (rauwe `/odom`, geen EKF, tegen externe IMU als grondwaarheid) juist een consistente **OVERSCHATTING** van 2,4-3,4× liet zien. Dit verschil in richting moet eerst verklaard worden voordat één schaalfactor-hypothese aangenomen kan worden. Twee dingen uit deze analyse zijn nog niet eerder gedaan en zijn nu **prioriteit 0** (vóór de EKF-yaw-bron-wijziging hieronder), omdat ze definitief onderscheid maken tussen "rf2o/LD06 zelf" en "gait-interactie":
   - **0a. Ontkoppelde LD06-test — UITGEVOERD (11 aug 2026), resultaat: geen vaste schaalfout.** MUTO opgetild (poten los van de grond), alleen LiDAR + rf2o actief (geen gait, geen EKF), robot met de hand rustig gedraaid tegen een vast referentiepunt. Twee metingen: (1) ~90° gedraaid → rf2o rapporteerde **-178,2°** (~2× overschatting), (2) 180° gedraaid tegen een referentielijn → rf2o rapporteerde **+156,7°** (0,87×, een ONDERSCHATTING). **De twee fouten wijzen tegengestelde kanten op** — dit sluit een vaste schaalfactor (zoals de "×0,667"-hypothese) uit. Het patroon komt overeen met een al langer bekende, aparte bevinding (zie PROBLEMS.md, "odom_fused wijkt tot een kwart slag af na snelle rotatie"): rf2o's scan-matching verliest **sporadisch** tracking bij grotere/snellere rotaties, met wisselende, niet-systematische afwijkingen tussen bijna-identieke tests. **Belangrijk:** dit gebeurde volledig los van de gait (robot opgetild, handmatig gedraaid) — de fout is dus geen gait-specifiek fenomeen, maar een intrinsieke betrouwbaarheidsbeperking van rf2o's scan-matching zelf bij rotatie in het algemeen. Dit bevestigt (met een preciezere verklaring) de eerder getrokken conclusie: rf2o's yaw is principieel onbetrouwbaar voor deze toepassing, gyro/IMU blijft de juiste primaire rotatiebron.
