...
**Update (zelfde dag, tweede analyse):** een onafhankelijke analyse van de gebruiker stelt een systematische schaalfout in rf2o zelf voor (i.p.v. een architectuurprobleem), met als voorgestelde vervolgtest: rf2o's rauwe `/odom` (zonder EKF) direct vergelijken met een fysiek gemeten hoek bij 90°/180°/360° in beide richtingen, plus de LD06 **losgekoppeld van de lopende MUTO** (handmatig/draaiplateau) testen om gait-vibratie volledig uit te sluiten. **Belangrijk verschil met onze eigen metingen:** de aangeleverde voorbeeldcijfers gaan uit van een ONDERSCHATTING (rf2o rapporteert ~2/3 van de werkelijke hoek, bv. 360°→240°), terwijl onze eigen `isolate_odom_vs_imu.py`-test (rauwe `/odom`, geen EKF, tegen externe IMU als grondwaarheid) juist een consistente **OVERSCHATTING** van 2,4-3,4× liet zien. Dit verschil in richting moet eerst verklaard worden voordat één schaalfactor-hypothese aangenomen kan worden. Twee dingen uit deze analyse zijn nog niet eerder gedaan en zijn nu **prioriteit 0** (vóór de EKF-yaw-bron-wijziging hieronder), omdat ze definitief onderscheid maken tussen "rf2o/LD06 zelf" en "gait-interactie":
   - **0a. Ontkoppelde LD06-test — UITGEVOERD (11 aug 2026), resultaat: geen vaste schaalfout, maar test methodologisch besmet.** MUTO opgetild (poten los van de grond), alleen LiDAR + rf2o actief (geen gait, geen EKF), **met de hand vastgehouden en gedraaid** tegen een vast referentiepunt. Twee metingen: (1) ~90° gedraaid → rf2o rapporteerde **-178,2°** (~2× overschatting), (2) 180° gedraaid tegen een referentielijn → rf2o rapporteerde **+156,7°** (0,87×, een ONDERSCHATTING). **De twee fouten wijzen tegengestelde kanten op** — dit sluit een vaste schaalfactor (zoals de "×0,667"-hypothese) uit, ongeacht de precieze ruisbron. **⚠️ Methodologisch voorbehoud (opgemerkt door de gebruiker direct na de test):** de LD06 scant 360° rondom, dus de vasthoudende armen/lichaam zaten binnen het scanbereik, mogelijk niet identiek gepositioneerd tussen beide metingen — dit kan zelf al scan-matching-correspondentiefouten veroorzaken, los van een echte rf2o- of gait-eigenschap. **De claim "dit bewijst gait-onafhankelijkheid" is dus NIET hard bewezen** — alleen "geen vaste schaalfactor" staat overeind. **✅ Schone herhaling UITGEVOERD, resultaat: rf2o zelf is betrouwbaar.** Torque uitgezet ná een verdiepte, vlakke splay-stand (poten op heupniveau, IK-gevalideerd), robot op een draaischijf zonder vasthouden gedraaid: 90° rechtsom → rf2o -89,58° (1,00×), 90° linksom terug → +91,63° (1,02×), round-trip-sluitfout ~2°. **Dit weerlegt de eerdere brede conclusie "rf2o's yaw is principieel onbetrouwbaar."** Zonder gait-vibratie én zonder hand-interferentie is rf2o vrijwel exact — de eerdere grote fouten komen dus specifiek van de gait-beweging en/of het vasthouden, niet van rf2o/LD06 zelf. Zie PROBLEMS.md voor het volledige traject. **Nieuwe openstaande vraag:** welk aspect van de gait precies (frequentie, amplitude, gait-fase) rf2o laat mislukken — nog niet apart geïsoleerd.
   - **0b. `/scan`-parameters controleren — UITGEVOERD, schoon:** `angle_min/max: -π/+π` (volledige 360°), `angle_increment: 0,00311 rad` (~2020 punten/scan, klopt met eerder bevestigd aantal), `scan_time: 0,098s` (~10,2 Hz, klopt met specificatie). Geen driver-configuratiefout gevonden. `ros2 topic echo /scan --once` — `angle_min`, `angle_max`, `angle_increment`, `scan_time` noteren en vergelijken met de LD06-specificatie, om een driver-configuratiefout uit te sluiten die nog niet gecontroleerd is (de eerdere broncode-analyse keek alleen naar rf2o's transform-compositie en QoS, niet naar de scan-geometrie zelf).
   - **Quaternion-yaw-extractie gecontroleerd (geen actie nodig):** `isolate_odom_vs_imu.py`'s `get_odom_yaw()` gebruikt `2·atan2(qz,qw)`, wat wiskundig identiek is aan de volledige yaw-formule zodra x=y=0 (het geval voor rf2o's zuivere yaw-quaternion). Geen bug, **met één caveat**: dit wrapt bij ±180°, dus bij een toekomstige test van 180° of meer in één keer kan schijnbare data-corruptie ontstaan die niets met rf2o's nauwkeurigheid te maken heeft — hou losse metingen bij een 90/180/360°-sweep daarom apart per hoek, niet cumulatief in één ongewrapte reeks.
