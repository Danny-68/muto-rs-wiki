...
1. **Nieuwe, onverklaarde zijwaartse drift tijdens recht-vooruit-lopen met phoenix_gait tripod, leidde tot een botsing tegen een deurpost (geen schade).** Dit is verrassend omdat phoenix_gait destijds uitvoerig getest is met minimale drift — de oorzaak is dus vermoedelijk iets van déze sessie, niet de gait zelf.
   - **✅ Software-regressie uitgesloten (byte-diff-geverifieerd, 11 aug 2026 avond):** `phoenix_gait.py` is byte-voor-byte identiek aan de op 10 augustus "standalone gevalideerde" versie (nu bewaard als beschermd origineel, zie `software/pi/gait/originals/phoenix_gait_ORIGINAL_VALIDATED_10aug2026.py` — **dit bestand nooit overschrijven**). `phoenix_driver.py`'s bewegingsgeneratie (`_step()`, decel+neutraal-stopsequentie) is functioneel ongewijzigd; de enige verschillen zijn nieuwe, losstaande features (STM32-yaw-publicatie, stop-debounce, `pose_settling`) die de foot-position-berekening niet raken.
   - **✅ Kwantitatieve vergelijking met de originele validatie (11 aug 2026 avond):** de 10-augustus-baseline (via `cmd_vel`/node, 5,3 cycli ≈ 50cm) gaf vooruit gemiddeld +1,30° (n=3) ≈ **~2,6°/meter** (directe-aanroep-baseline: +2,80° ≈ ~5,6°/meter). Vandaag's 1m-test (met correcte 27s wachttijd) gaf via de X/Y-verhouding in `/odom_fused` een koershoek van ~9,8° over ~0,8m ≈ **~12,25°/meter** — **2-4× meer drift dan het gevalideerde origineel.** Dit is dus geen normale spreiding maar een reëel, kwantificeerbaar verschil.
   - **Conclusie: de oorzaak is fysiek, niet software.** Met software uitgesloten en de drift-toename kwantitatief bevestigd, blijven twee kandidaten over:
   ...
   - **Vervolgtest (nog te doen):** een schone rechte-lijn-test op een aantoonbaar vlakke plek, ver van drempels/obstakels, met volledige 27s+ wachttijd — bij voorkeur ook op de oorspronkelijke testlocatie van 10 augustus, om vloer vs. mechanisch verder te scheiden.
