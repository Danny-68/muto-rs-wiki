...
- **Vervolg, zelfde sessie: geen doorlopende verslechtering bevestigd — herstel opgetreden.** Batterij drie keer stabiel bevestigd op 7,7V/92% (één eenmalige 0,0V-uitlezing bleek een seriële-communicatieglitch vlak na het stoppen van `phoenix_driver.py`, geen echte spanningsval — herhaalde uitlezingen direct daarna weer stabiel). Drie verse standaard-tripod-rotatietests direct daarna: **0,82× / 0,83× / 0,76×** — allemaal terug in de normale bandbreedte (0,74-0,85×) van Pad A/C, geen verdere achteruitgang.
- **Werkconclusie:** de eerdere duik naar 0,40-0,60× viel samen met metingen waarbij de fysieke rotatiehoek zelf kromp (9,6°→6,0°→4,0° tijdens de zachte-gait-reeks) — kleine hoeken versterken ruis in de ratio-berekening, wat een waarschijnlijkere verklaring is dan een echt, aanhoudend batterij/voedingsprobleem. **Nog niet getest:** spanning ONDER belasting (tijdens een actieve burst zelf, i.p.v. alleen ervoor/erna) — de gebruiker opperde een gedeelde-voedingsrail-hypothese (Pi + MUTO-servo's op dezelfde batterij, mogelijke spanningsdip bij 18 gelijktijdig actieve servo's) die hiermee nog niet is uitgesloten, maar door de huidige data ook niet wordt ondersteund. **Voorlopig geen sterke aanwijzing dat een tweede/gescheiden batterij nodig is** — als dit patroon (dip bij kleine hoeken) zich blijft herhalen, is dat eerder een reden om kleine-hoek-metingen als onbetrouwbaar te behandelen dan om de voeding aan te passen.
- **Consequentie voor lopend onderzoek:** de fijnmazige Pad B/C-vergelijking blijft het beste te doen met voldoende grote rotatiehoeken (>10°) per meting, om deze kleine-hoek-ruis te vermijden.
